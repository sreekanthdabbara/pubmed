...
def results_page():
    ...
    if search_id and search_id in _search_cache:
        ...
        for kw, df in sorted_results.items():
            ...
            # Clean NaN/inf values — prevent JSON parse errors in JS
            df_clean = df.fillna('').copy()
            for col in df_clean.select_dtypes(include=['float','object']).columns:
                df_clean[col] = df_clean[col].astype(str).replace({'nan':'','None':'','inf':'','<NA>':''})

            records    = df_clean.to_dict('records')
            free_count = sum(1 for r in records if str(r.get('is_free_pmc','')).lower() == 'true')
            total_free_articles += free_count
            total_ncbi_articles += count

            keyword_summary.append({
                'keyword': kw, 'count': count,
                'ncbi_total': count, 'fetched': count,
                'has_more': False, 'free_count': free_count,
                'results': records,
            })
...
def search_url():
    ...
    FILTER_MAP = {
        ...
        # Use [Filter] tag not [Publication Type] — matches PubMed web internal behavior
        'pubt.adaptiveclinicaltrial':         '"adaptive clinical trial"[Filter]',
        'pubt.address':                       '"address"[Filter]',
        'pubt.biography':                     '"biography"[Filter]',
        'pubt.booksanddocuments':             '"books and documents"[Filter]',
        'pubt.casereports':                   '"case reports"[Filter]',
        'pubt.clinicalstudy':                 '"clinical study"[Filter]',
        'pubt.clinicaltrial':                 '"clinical trial"[Filter]',
        'pubt.clinicaltrialprotocol':         '"clinical trial protocol"[Filter]',
        'pubt.clinicaltrialphase1':           '"clinical trial, phase i"[Filter]',
        'pubt.clinicaltrialphase2':           '"clinical trial, phase ii"[Filter]',
        'pubt.clinicaltrialphase3':           '"clinical trial, phase iii"[Filter]',
        'pubt.clinicaltrialphase4':           '"clinical trial, phase iv"[Filter]',
        'pubt.clinicaltrialphaseii':          '"clinical trial, phase ii"[Filter]',
        'pubt.clinicaltrialphaseiii':         '"clinical trial, phase iii"[Filter]',
        'pubt.clinicaltrialphaseiv':          '"clinical trial, phase iv"[Filter]',
        'pubt.clinicaltrial,veterinary':      '"clinical trial, veterinary"[Filter]',
        'pubt.collectedwork':                 '"collected work"[Filter]',
        'pubt.comment':                       '"comment"[Filter]',
        'pubt.comparativestudy':              '"comparative study"[Filter]',
        'pubt.conferenceproceedings':         '"conference proceedings"[Filter]',
        'pubt.consensusstatement':            '"consensus development conference"[Filter]',
        'pubt.controlledclinicaltrial':       '"controlled clinical trial"[Filter]',
        'pubt.dataset':                       '"dataset"[Filter]',
        'pubt.editorial':                     '"editorial"[Filter]',
        'pubt.equivalencetrial':              '"equivalence trial"[Filter]',
        'pubt.evaluationstudy':               '"evaluation study"[Filter]',
        'pubt.guideline':                     '"guideline"[Filter]',
        'pubt.historicalarticle':             '"historical article"[Filter]',
        'pubt.interview':                     '"interview"[Filter]',
        'pubt.letter':                        '"letter"[Filter]',
        'pubt.meta-analysis':                 '"meta-analysis"[Filter]',
        'pubt.multicenterstudy':              '"multicenter study"[Filter]',
        'pubt.news':                          '"news"[Filter]',
        'pubt.observationalstudy':            '"observational study"[Filter]',
        'pubt.practiceguideline':             '"practice guideline"[Filter]',
        'pubt.pragmaticclinicaltrial':        '"pragmatic clinical trial"[Filter]',
        'pubt.preprint':                      '"preprint"[Filter]',
        'pubt.randomizedcontrolledtrial':     '"randomized controlled trial"[Filter]',
        'pubt.review':                        '"review"[Filter]',
        'pubt.scopingreview':                 '"systematic review"[Filter]',
        'pubt.systematicreview':              '"systematic review"[Filter]',
        'pubt.twinstudy':                     '"twin study"[Filter]',
        'pubt.validationstudy':               '"validation study"[Filter]',
        ...
        for f in filters:
            ...
            if f in FILTER_MAP:
                ...
                if clause in seen_clauses:
                    continue
                seen_clauses.add(clause)
                if '[Filter]' in clause and '[Publication Type]' not in clause and 'pubt.' in f:
                    pub_type_clauses.append(clause)
                elif '[Publication Type]' in clause:
                    pub_type_clauses.append(clause)
                else:
                    other_clauses.append(clause)
            else:
...
def _extract_zip_articles(zip_bytes: bytes) -> list:
    """Extract articles from a ZIP of EpiLite-generated PDFs."""
    import zipfile as _zf
    ...
    try:
        with _zf.ZipFile(io.BytesIO(zip_bytes)) as zf:
            ...
            print(f"[zip_extract] {len(pdf_files)} PDFs found")
            ...
            for pdf_name in pdf_files[:100]:
                try:
                    ...
                    # Title: second non-empty line after "EpiLite"
                    lines = [l.strip() for l in text.split('\n') if l.strip()]
                    title = ''
                    ...
                    print(f"[zip_extract] PDF error {pdf_name}: {pe}")

        print(f"[zip_extract] Extracted {len(articles)} articles")
    except Exception as e:
        print(f"[zip_extract] ZIP error: {e}")
    return articles
...
def upload_session():
    ...
    try:
        if fname.endswith('.csv'):
            ...
            articles = _extract_zip_articles(f.read())
        else:
        ...
        if token and token in _file_sessions:
            ...
            try:
                if fname.endswith('.csv'):
                    ...
                    # Extract PDFs from ZIP — same logic as upload_session
                    articles = _extract_zip_articles(f.read())
            except Exception as e:
                return jsonify({'error': f'Could not read file: {e}'}), 400
